...
export default function EvalCompletionScreen() {
  const { state } = useEvaluation();
  const dialectLabel = state.selectedDialect
    ? getDialectLabel(state.selectedDialect)
    : null;
  const comparisonsDone = state.comparisonResponses.length;
    ...
    <div className="min-h-screen bg-neutral-100 px-4 py-12">
      <div className="mx-auto max-w-lg">
        ...
        <div className="mt-6 space-y-4 text-sm leading-relaxed text-neutral-700">
          <p>
            Your translation comparison choices were submitted ({comparisonsDone} story
            {comparisonsDone === 1 ? "" : " reviews"}, including any stories marked as not yet
            available).
          </p>
          <p>
          ...
          <dl className="space-y-3">
            {dialectLabel ? (
              <div className="flex justify-between gap-4">
                <dt className="text-neutral-600">Declared dialect</dt>
            ...
            <div className="flex justify-between gap-4">
              ...
              <dd className="text-right font-medium text-neutral-900">
                ...
                {evaluationQuizQuestions.length}
              </dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-neutral-600">Comparisons submitted</dt>
              <dd className="text-right font-medium text-neutral-900">{comparisonsDone}</dd>
            </div>
          </dl>
        </div>

        <p className="mt-8 text-sm text-neutral-500">
          You can close this window now. Thank you for contributing to this evaluation study.
        </p>
      </div>
    </div>
  );
